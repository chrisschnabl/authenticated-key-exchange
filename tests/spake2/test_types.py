...
class TestSPAKE2Types(unittest.TestCase):
    @parameterized.expand(
        [
            ...
            (
                "client_confirmation_invalid",
                ClientConfirmation,
                os.urandom(MAC_SIZE - 1),
                None,
                False,
            ),
            ("server_confirmation_valid", ServerConfirmation, os.urandom(MAC_SIZE), None, True),
            (
                "server_confirmation_invalid",
                ServerConfirmation,
                os.urandom(MAC_SIZE - 1),
                None,
                False,
    ...
    def test_keyset_construction(self) -> None:
        test_keys = [Key(value=os.urandom(KEY_SIZE)) for _ in range(4)]

        keyset = KeySet(ke=test_keys[0], ka=test_keys[1], kcA=test_keys[2], kcB=test_keys[3])
    ...
    def test_inheritance_relationships(
        self, name: str, subclass: type[BaseModel], parent_class: type[BaseModel]
    ) -> None:
        if parent_class == Key:
