...
class TestHashing(unittest.TestCase):
    def test_hash_large_input(self) -> None:
        result = hash(b"x" * 10000000)
        self.assertEqual(len(result), 64)

    def test_hash_null_bytes(self) -> None:
    ...
    def test_hkdf_lengths_and_edge_cases(self, key: bytes, info: bytes, length: int) -> None:
        result = hkdf(key, info, length)
        self.assertEqual(len(result), length)
        self.assertIsInstance(result, bytes)

    def test_hkdf_minimum_length(self) -> None:
        # Must be at least 16
        with self.assertRaises(Exception):
            hkdf(b"key", b"info", 1)

    def test_hkdf_zero_length(self) -> None:
        with self.assertRaises(Exception):
            hkdf(b"key", b"info", 0)

    def test_hkdf_negative_length(self) -> None:
        with self.assertRaises(Exception):
            hkdf(b"key", b"info", -1)
    ...
    def test_hkdf_output_difference(
        self, key1: bytes, key2: bytes, info: bytes, length: int
    ) -> None:
    ...
    def test_hkdf_output_consistency(self) -> None:
        for _ in range(5):
            key = os.urandom(16)
            info = os.urandom(16)
            length = 32
    ...
    def test_hkdf_parameters(self, mock_kdf: MagicMock) -> None:
    ...
    def test_hkdf_with_unicode_info(self) -> None:
        try:
            info = "测试信息".encode()
    ...
    def test_hkdf_with_different_encodings(self) -> None:
        key = b"key"
        info_utf8 = "特殊字符".encode()
