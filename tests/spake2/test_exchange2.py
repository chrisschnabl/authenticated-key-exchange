...
class SPAKE2TestVectorVerification:
    def __init__(self):
        ...
        self.test_vectors = {
            ...
            "Z": binascii.unhexlify(
                "04bbfce7dd7f277819c8da21544afb7964705569bdf12fb92aa388059408d5"
                "0091a0c5f1d3127f56813b5337f9e4e67e2ca633117a4fbd559946ab474356c41839"
            ),
            "V": binascii.unhexlify(
                "0458bf27c6bca011c9ce1930e8984a797a3419797b936629a5a937cf2f11c8"
                "b9514b82b993da8a46e664f23db7c01edc87faa530db01c2ee405230b18997f16b68"
            ),
            "TT": binascii.unhexlify(
                "38000000000000005350414b45322b2d503235362d5348413235362d484b4"
        ...
        self.M_UNCOMPRESSED = binascii.unhexlify(
        ...
        self.N_UNCOMPRESSED = binascii.unhexlify(
    ...
    def verify_prover_calculation(self) -> Dict[str, bool]:
        ...
        # Step 2: Verify shareP calculation (check uncompressed format)
        expected_shareP = self.test_vectors["shareP"]
        results["shareP_correct"] = expected_shareP[0] == 0x04
        
        # Step 3: Verify Z calculation (check uncompressed format)
        expected_Z = self.test_vectors["Z"]
        results["Z_correct"] = expected_Z[0] == 0x04
        ...
        # Step 6: Verify HMAC calculation for prover
    ...
    def verify_verifier_calculation(self) -> Dict[str, bool]:
        ...
        # Step 2: Verify shareV calculation (check uncompressed format)
        expected_shareV = self.test_vectors["shareV"]
        results["shareV_correct"] = expected_shareV[0] == 0x04
        
        # Step 3: Verify key derivation for verifier
        expected_K_confirmV = self.test_vectors["K_confirmV"]
        results["K_confirmV_correct"] = len(expected_K_confirmV) == 32
        
        # Step 4: Verify HMAC calculation for verifier
...
# ---------------------------------------------------------------------------
class TestSPAKE2VectorVerification(unittest.TestCase):
    def setUp(self):
        self.spa = SPAKE2TestVectorVerification()

    def test_prover_calculation(self):
        results = self.spa.verify_prover_calculation()
        for key, value in results.items():
            with self.subTest(test=key):
                self.assertTrue(value, f"Prover test failed: {key}")

    def test_verifier_calculation(self):
        results = self.spa.verify_verifier_calculation()
        for key, value in results.items():
            with self.subTest(test=key):
                self.assertTrue(value, f"Verifier test failed: {key}")

    def test_overall_verification(self):
        # Ensure that all sub-tests pass overall
        prover_results = self.spa.verify_prover_calculation()
        verifier_results = self.spa.verify_verifier_calculation()
        all_results = list(prover_results.values()) + list(verifier_results.values())
        self.assertTrue(all(all_results), "Overall SPAKE2 test vector verification failed.")
