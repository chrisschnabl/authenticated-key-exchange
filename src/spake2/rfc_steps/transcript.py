...
from spake2.spake_types import AdditionalData, Identity, Key, KeySet, Mac, Transcript
...
def create_transcript(idA: Identity, idB: Identity, pA: Key, pB: Key, K: Key, w: int) -> Transcript:
    ...
    transcript = (
        len(idA_bytes).to_bytes(8, byteorder="little")
        + idA_bytes
        + len(idB_bytes).to_bytes(8, byteorder="little")
        + idB_bytes
        + len(pA_bytes).to_bytes(8, byteorder="little")
        + pA_bytes
        + len(pB_bytes).to_bytes(8, byteorder="little")
        + pB_bytes
        + len(K_bytes).to_bytes(8, byteorder="little")
        + K_bytes
        + len(w_bytes).to_bytes(8, byteorder="little")
        + w_bytes
    )
...
def derive_keys(transcript: Transcript, aad: AdditionalData) -> KeySet:
    ...
    kdf_output = hkdf(key=ka, info=b"ConfirmationKeys" + aad.value, length=64)
